...
(() => {
    ...
    class Load {
        ...
        createRuleText() {
            ...
            // １ページ目
            if (this._page === 1) {
                titleText.push(`日本式ポーカー（クローズド・ポーカー）<br><br>`);
                sentenceText.push(`チップやベット・アンティ・フォールドなどのない簡易なポーカーになります。<br>
           左側の役一覧を参考に手札をそろえて、<br>
           より強い役を作ったプレイヤーが勝ちとなります。<br><br>
           
           （次のページに続きます。）`);
            }
            // ２ページ目
            if (this._page === 2) {
                titleText.push(`「ゲームの進め方」について<br><br>`);
                sentenceText.push(`「カードを配る」ボタンをクリックすると、すべてのプレイヤーにカードが 5 枚ずつ配られます。<br><br>

          カードが配られたら、「ポーカー開始！」「カードを配りなおす」ボタンが表示されます。<br><br>
  
          「ポーカー開始！」ボタンでゲームが始まります。<br>
          「カードを配りなおす」ボタンでカードを配り直します。<br><br>

          Player1 → Player2 → Player3 → You の順番でゲームが進みます。<br><br>
           ...
           （次のページに続きます。）`);
            ...
            if (this._page === 3) {
                ...
                sentenceText.push(`自分のターンになったら、手札から不要なカードを捨て札に出して、<br>
           積み札から捨てた枚数分 補充することができます。<br>
           カードの補充ができるのは一度だけとなります。<br><br>

           補充が終わったら次のプレイヤーのターンになります。<br>
           補充しない場合は そのまま 次のプレイヤーのターンになります。<br><br>

           すべてのプレイヤーのターンが終わったら、一斉に手札を公開します。<br>
           そしてお互いの役を見比べて、順位が確定します。<br><br>

           自分の手札がどの役に該当するかについては、<br>
           左側の役一覧において緑の背景色が適用されますのでご確認ください。<br><br>
      ...
      （次のページに続きます。）`);
            ...
            if (this._page === 4) {
                titleText.push(`自分のターンになったとき<br><br>`);
                sentenceText.push(`手札から不要なカードをクリックすると、クリックしたカードが少し上に移動します。<br>
          不要なカードをすべてクリックしたあと、「カードを交換する」ボタンをクリックすると、<br>
          不要なカードを捨て札に出して、積み札から捨てた枚数分 補充されます。<br><br>

          不要なカードが１枚もなければ「この手札で勝負する」ボタンをクリックすると、<br>
          そのままターンを終えます。<br><br>
      ...
      （次のページに続きます。）`);
            ...
            // ５ページ目
                ...
                titleText.push(`「役の種類」について（強い順に記載）<br><br>`);
           ...
           　フルハウスの強さはスリーカードの数字によって判断されます。<br>
           　同じ数字のスリーカードを相手も持っていた場合はワンペアの数字によって判断されます。<br><br>
           ...
           （次のページに続きます。）`);
            ...
            // ８ページ目
            else if (this._page === 8) {
                titleText.push(`「順位の決め方」について<br><br>`);
                sentenceText.push(`ポーカーの役が強いほど、高い順位が割り当てられます。<br><br>

          ポーカーの役が同じ場合は、そろえた役の中の数字の強さで順位が決まります。<br><br>

          そろえた役の中の数字も同じ場合は、役に属さないカードの数字の強さで順位が決まります。<br><br>

          役に属さないカードの数字も同じ<br>
          （※同じ役で、自分の手札と相手の手札のカードの数字がすべて同じ）場合は、<br>
          同一の順位が割り当てられます。<br><br>

          （次のページに続きます。）`);
            }
            // ９ページ目
            else if (this._page === 9) {
                titleText.push(`「カードの数字の強さ」について<br><br>`);
                sentenceText.push(`ポーカーは「A」が一番強く、<br>
           「K」→「Q」→「J」→「10」と順番に弱くなっていき、「2」が一番弱いカードとなります。<br><br>

           ただし、例外として、手札が「 A 2 3 4 5 」となっている場合は、「A」が一番弱いカードになります。<br><br>`);
                titleText.push(`「ストレート系の役の注意点」について<br><br>`);
                sentenceText.push(`「 A 2 3 4 5 」  これはストレートになります。この場合 A が最も弱くなります。<br><br>

           「 10 J Q K A 」 これはストレートになります。この場合 A が最も強くなります。<br><br>

           「 J Q K A 2 」  これはストレートではありませんので、ご注意ください。`);
            }
            // ルール説明ページ 生成
    ...
    class Poker {
        constructor() {
            ...
            setTimeout(() => {
                // ストックの位置にボタンを配置
                this._tool.createDoubleLineBtn(document.querySelector('.stock'), 'startPokerBtn', 'ポーカー<br>開 始！');
                const startPokerBtn = document.querySelector('.startPokerBtn');
                this._tool.nullCheck(startPokerBtn);
                startPokerBtn.addEventListener('click', () => {
                    // ボタンを消去
                    const stock_div = document.querySelectorAll('.stock>div');
                    stock_div.forEach((div) => {
                        div.remove();
                    });
                    // メッセージを非表示
                ...
                this._tool.createDoubleLineBtn(document.querySelector('.stock'), 'backToStartBtn', 'カードを<br>配り直す');
                const backToStartBtn = document.querySelector('.backToStartBtn');
                this._tool.nullCheck(backToStartBtn);
                backToStartBtn.addEventListener('click', () => {
                    this.click_backToStartBtn();
                });
            }, 11000);
        }
        createTrump() {
